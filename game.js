...
/* skill pool (includes new skills) */
const SKILL_POOL = [
...
const BOSS_ABILITIES = [
  {
    id: 'reincarnation',
    name: '♻ 転生',
    desc: '破壊時に (破壊直前値 % 指の最大値) で復活（mod=0 なら破壊のまま）',
...
function resetAllProgress(){
  // used for full reset via Reset button
...
function resetFullGameToTitle(){
  // keep unlocked & best, reset run variables and return to title
  gameState.stage = 1;
  gameState.isEndless = false;
  gameState.isGameClear = false;
  gameState.powerLevel = 0;
...
function initGame(){
  ...
  // attach handlers (overwrite existing to avoid duplicates)
...
function startBattle(){
  ...
  // endless powerLevel increment at stage start (13を含む)
  ...
  // wait for equip confirmation (so possession etc. apply after commit)
...
/* ---------- assign boss ability (may be composite if stage >=12) ---------- */
...
function showEquipSelection(){
  ...
  confirm.textContent = '確定'; confirm.style.marginLeft = '8px';
...
function playerAttack(targetSide){
  ...
  let newVal = curEnemy + added; if(!Number.isFinite(newVal)) newVal = 0;
  const destroyThreshold = getDestroyThreshold(true);
  let destroyed = false; const attemptedValue = newVal;
  if(Number(newVal) >= Number(destroyThreshold)){
    newVal = 0; destroyed = true; animateDestroy(targetEl); playSE('destroy', 0.9);
    if(gameState.isBoss && gameState.bossAbility && typeof gameState.bossAbility.onHandDestroyed === 'function'){
      try { gameState.bossAbility.onHandDestroyed(targetSide, attemptedValue); } catch(e){} // boss may revive by setting gameState.enemy[targetSide]
    }
  } else { if(newVal > HARD_CAP) newVal = HARD_CAP; }
  gameState.enemy[targetSide] = newVal;
  handleCounter(false, attackerKey, true, targetSide);
  if(destroyed && hasEquipped('chain')){ const lvl = getEquippedLevel('chain'); applyTurnBuff('chainBoost', lvl, 1); const tb = gameState.turnBuffs[gameState.turnBuffs.length - 1]; if(tb) tb.payload = { type:'chainBoost', value: lvl }; messageArea.textContent = `チェイン発動！次の攻撃が +${lvl}されます`; }
  clearHandSelection(); gameState.playerTurn = false; updateUI(); flashScreen();
...
function enemyTurn(){
  if(gameState.inBossReward) return;
  if(gameState.isBoss && gameState.bossAbility && typeof gameState.bossAbility.onEnemyTurnStart === 'function'){ try { gameState.bossAbility.onEnemyTurnStart(); } catch(e){} }
  ...
  let attackValue = toNum(gameState.enemy[from]); attackValue += computeEnemyAttackBonus(from); attackValue += (gameState.enemyBase && Number.isFinite(Number(gameState.enemyBase.baseAttack))) ? Number(gameState.enemyBase.baseAttack) : 0;
  ...
  if(wasDestroyed){ newVal = 0; animateDestroy(targetEl); playSE('destroy', 0.9); } else { if(newVal > HARD_CAP) newVal = HARD_CAP; }
...
function triggerGameClear(){
  gameState.isGameClear = true; gameState.isEndless = false; updateBestStage(); showClearScreen();
}
...
/* ---------- hover helpers already implemented above ---------- */

/* ---------- winner/loser handled above ---------- */
