/* game.js — 完全置換版（BattleEngine 統合・転生/ポゼッション/超耐久修正済み） */
...
class BattleEngine {
  ...
  emit(name, ...args){ this._eventLoopCount++; if(this._eventLoopCount>this._maxEventLoop){ console.warn('event loop exceeded'); return; } const handlers=(this._events[name]||[]).slice(); for(const h of handlers){ try{ h(...args);}catch(e){console.error(e);} } }
  _resetEventLoopCount(){ this._eventLoopCount=0; }
  _enterCritical(){ if(this._reentrancyGuard) throw new Error('reentrancy'); this._reentrancyGuard=true; }
  _exitCritical(){ this._reentrancyGuard=false; }
  clampHandVal(v){ const mx=(this.state.baseStats && Number.isFinite(Number(this.state.baseStats.maxFinger)))?Number(this.state.baseStats.maxFinger):5; if(!Number.isFinite(Number(v))) return 0; return Math.max(0, Math.min(v, Math.max(mx, v))); }
  ...
  registerBossAbility(id, handler){ this.bossRegistry[id]=handler; }
  assignBossAbility(id){
    const h=this.bossRegistry[id]; if(!h) return;
    try{ if(typeof h.apply==='function') h.apply(this, this.state); this.appliedBossAbilities.push(id); this.emit('bossAbilityAssigned', id); }catch(e){console.error(e);}
  }
  clearBossAbilities(){
    this.appliedBossAbilities=[]; this.state.bossEnemyThresholdMultiplier=1; this.state.enemyHasThirdHand=false; this.state.bossTurnCount=0; this.emit('bossAbilitiesCleared');
  }
  _callBossHook(hook, ...args){
    for(const id of this.appliedBossAbilities.slice()){
      ...
      if(h && typeof h[hook] === 'function'){
        try{ h[hook](this, ...args); }catch(e){ console.error(e); }
      }
    }
  }

  _installDefaultBossAbilities(){
    // reincarnation: onHandDestroyed => attemptedValue % maxFinger, revive if mod != 0
    this.registerBossAbility('reincarnation', {
      name:'reincarnation',
      desc:'破壊後に (attemptedValue % maxFinger) で復活（0なら破壊）',
      onHandDestroyed: (engine, side, attemptedValue) => {
        try {
          ...
          if(mod !== 0){
            ...
            try{ if(typeof showPopupText === 'function') showPopupText(el, `復活 ${mod}`, '#ffd166'); }catch(e){}
            try{ if(typeof playSE === 'function') playSE('skill'); }catch(e){}
  ...
  getDestroyThreshold(attackerIsPlayer = true){
    ...
    if(attackerIsPlayer) thresholdRaw = thresholdRaw * (this.state.bossEnemyThresholdMultiplier || 1);
    else thresholdRaw = thresholdRaw * (this.state.playerBattleModifiers && this.state.playerBattleModifiers.thresholdMultiplier ? this.state.playerBattleModifiers.thresholdMultiplier : 1);
    ...
    return Math.max(2, Math.round(Number.isFinite(Number(thresholdRaw)) ? thresholdRaw : 5));
  }
  ...
  _attack(attackerIsPlayer, attackerSide, targetSide){
    if(this._reentrancyGuard){ console.warn('attack during critical'); return; }
    this._enterCritical();
    try{
      ...
      if(attackerVal <= 0){ this._exitCritical(); return; }

      let baseAtk = attackerVal;
      if(attackerIsPlayer) baseAtk += this._computePlayerAttackBonus(attackerSide);
      else baseAtk += this._computeEnemyAttackBonus(attackerSide);
      ...
      try{ if(typeof playSE === 'function') playSE('attack'); }catch(e){}
    ...
    } finally { this._exitCritical(); this._resetEventLoopCount(); }
  }

  enemyTurn(){
    if(this._reentrancyGuard){ console.warn('enemyTurn during critical'); return; }
    this._enterCritical();
    try{
      ...
      (this.state.enemySkills || []).forEach(skill => {
        try{
          ...
          if(skill.id === 'double'){ if(Math.random()<0.35){ this.state.enemyDoubleMultiplier = 1 + skill.level; skill.remainingCooldown = 2; } }
          if(skill.id === 'regen'){ this._applyRegenToUnit(true, skill.level); }
          if(skill.id === 'fortify' && Math.random()<0.25){ this._applyEnemyTurnBuff('fortify', skill.level, 2*skill.level); skill.remainingCooldown = 3; }
          if(skill.id === 'chain' && Math.random()<0.25){ this._applyEnemyTurnBuff('chain', skill.level, 1); skill.remainingCooldown=2; }
          if(skill.id === 'disrupt' && Math.random()<0.35){
            const candidates=['left','right'].filter(k=>this.clampHandVal(this.state.player[k])>0);
            if(candidates.length>0){ const target=candidates[rand(0,candidates.length-1)]; const amount=1+skill.level; this.state.player[target]=this.clampHandVal(safeDecrease(this.state.player[target], amount)); skill.remainingCooldown=2; }
          }
          if(skill.id === 'teamPower' && Math.random()<0.2){ this._applyEnemyTurnBuff('teamPower', skill.level, 2*skill.level); skill.remainingCooldown=3; }
        }catch(e){ console.error(e); }
      });
      ...
      if(aliveEnemy.length===0 || alivePlayer.length===0){ this._exitCritical(); return; }
  ...
  _tickTurnBuffs(){ (this.state.turnBuffs||[]).forEach(tb=>tb.remainingTurns=Math.max(0,tb.remainingTurns-1)); this.state.turnBuffs=(this.state.turnBuffs||[]).filter(tb=>tb.remainingTurns>0); (this.state.equippedSkills||[]).forEach(s=>{ if(s.remainingTurns>0) s.remainingTurns=Math.max(0,s.remainingTurns-1); }); }
  _applyEnemyTurnBuff(skillId, level, duration){ let payload={}; if(skillId==='fortify') payload={type:'enemyGuardBoost',value:level}; else if(skillId==='teamPower') payload={type:'teamPower', value:level}; else payload={type:skillId, value:level}; this.state.enemyTurnBuffs=this.state.enemyTurnBuffs||[]; this.state.enemyTurnBuffs.push({ skillId, remainingTurns: duration, payload}); }
  _tickEnemyTurnBuffs(){ (this.state.enemyTurnBuffs||[]).forEach(tb=>tb.remainingTurns=Math.max(0,tb.remainingTurns-1)); this.state.enemyTurnBuffs=(this.state.enemyTurnBuffs||[]).filter(tb=>tb.remainingTurns>0); (this.state.enemySkills||[]).forEach(s=>{ if(s.remainingCooldown && s.remainingCooldown>0) s.remainingCooldown=Math.max(0,s.remainingCooldown-1); }); }
  _applyRegenToUnit(isEnemy, level){ const targetObj = isEnemy ? this.state.enemy : this.state.player; let sides = ['left','right']; if(isEnemy && this.state.enemyHasThirdHand) sides.push('third'); sides = sides.filter(k=>this.clampHandVal(targetObj[k])>0); if(sides.length===0) return; for(let i=0;i<level;i++){ const r = sides[rand(0,sides.length-1)]; const cur=this.clampHandVal(targetObj[r]); const newVal=safeDecrease(cur,1); targetObj[r]=newVal; const el = isEnemy ? (r==='left'?hands.enemyLeft:(r==='right'?hands.enemyRight:hands.enemyThird)) : (r==='left'?hands.playerLeft:hands.playerRight); try{ if(typeof showPopupText==='function') showPopupText(el, `-${1}`, '#ff9e9e'); }catch(e){} } }
  _handleCounter(attackerIsEnemy, attackerSide, targetIsEnemy, targetSide){
    ...
    if(attackerIsEnemy){
      const cur=this.clampHandVal(this.state.enemy[attackerSide]); this.state.enemy[attackerSide]=Math.min(HARD_CAP, cur+counterLevel); const el = attackerSide==='left'?hands.enemyLeft:hands.enemyRight; try{ if(typeof showPopupText==='function') showPopupText(el, `+${counterLevel}`, '#ffd166'); }catch(e){} 
    } else {
      const cur=this.clampHandVal(this.state.player[attackerSide]); this.state.player[attackerSide]=Math.min(HARD_CAP, cur+counterLevel); const el = attackerSide==='left'?hands.playerLeft:hands.playerRight; try{ if(typeof showPopupText==='function') showPopupText(el, `+${counterLevel}`, '#ffd166'); }catch(e){} 
    }
  }
  _computePlayerAttackBonus(handKey){ let bonus=0; (this.state.equippedSkills||[]).forEach(s=>{ if(s.type!=='passive') return; if(s.id==='power') bonus+=s.level; if(s.id==='berserk' && this.clampHandVal(this.state.player[handKey])===4) bonus+=s.level*2; }); (this.state.turnBuffs||[]).forEach(tb=>{ if(tb.payload){ if(tb.payload.type==='chainBoost') bonus+=tb.payload.value; if(tb.payload.type==='teamPower') bonus+=tb.payload.value; } }); const baseAtk=(this.state.baseStats && this.state.baseStats.baseAttack)?Number(this.state.baseStats.baseAttack):0; const atkMul=(this.state.playerBattleModifiers && this.state.playerBattleModifiers.attackMultiplier)?this.state.playerBattleModifiers.attackMultiplier:1; bonus+=baseAtk*atkMul; return bonus; }
  _computeEnemyAttackBonus(attackerHandKey){ let bonus=0; (this.state.enemySkills||[]).forEach(s=>{ if(s.type!=='passive') return; if(s.id==='power') bonus+=s.level; if(s.id==='berserk' && this.clampHandVal(this.state.enemy[attackerHandKey])===4) bonus+=s.level*2; }); (this.state.enemyTurnBuffs||[]).forEach(tb=>{ if(tb.payload && tb.payload.type==='chainBoost') bonus+=tb.payload.value; if(tb.payload && tb.payload.type==='teamPower') bonus+=tb.payload.value; }); return bonus; }
  _computeDefenseForTarget(targetIsEnemy){ let reduction=0; if(targetIsEnemy){ (this.state.enemySkills||[]).forEach(s=>{ if(s.id==='guard') reduction+=s.level; }); (this.state.enemyTurnBuffs||[]).forEach(tb=>{ if(tb.payload && (tb.payload.type==='enemyGuardBoost' || tb.payload.type==='guardBoost')) reduction+=tb.payload.value; }); } else { (this.state.equippedSkills||[]).forEach(s=>{ if(s.id==='guard') reduction+=s.level; }); (this.state.turnBuffs||[]).forEach(tb=>{ if(tb.payload && tb.payload.type==='guardBoost') reduction+=tb.payload.value; }); } const baseDef=(this.state.baseStats && this.state.baseStats.baseDefense)?Number(this.state.baseStats.baseDefense):0; const defMul=(this.state.playerBattleModifiers && this.state.playerBattleModifiers.defenseMultiplier)?this.state.playerBattleModifiers.defenseMultiplier:1; if(!targetIsEnemy) reduction+=baseDef*defMul; return reduction; }
  _getSkillLevelOnUnit(isEnemy, skillId){ if(isEnemy){ if(!this.state.enemySkills) return 0; const s=this.state.enemySkills.find(x=>x.id===skillId); return s? (s.level||1):0; } else { const s=(this.state.equippedSkills||[]).find(x=>x.id===skillId); return s? (s.level||1):0; } }
...
/* ---------- Game code (integrated with engine) ---------- */
...
const SKILL_POOL = [
  ...
  { id:'split',     type:'active',  baseDesc:'片手のみ生存かつ値≥2の時、その手を半分にし復活させる', name:'✂ 分割', rarity:'common'}
];
...
/* expose registration convenience for game to use default boss IDs */
engine.registerBossAbility('reincarnation', engine.bossRegistry['reincarnation']);
engine.registerBossAbility('thirdHand', engine.bossRegistry['thirdHand']);
engine.registerBossAbility('fortress', engine.bossRegistry['fortress']);
engine.registerBossAbility('split', engine.bossRegistry['split']);

/* ---------- seeding & reset ---------- */
function seedInitialUnlocks(){ gameState.unlockedSkills = [{ id:'power', level:1 }, { id:'guard', level:1 }]; saveUnlocked(); }
function resetGame(){ if(!confirm('スキルのアンロックを初期状態にリセットします。\nよろしいですか？')) return; try { localStorage.removeItem(STORAGE_KEY); } catch(e){} seedInitialUnlocks(); gameState.stage = 1; gameState.isBoss = false; gameState.player = { left:1, right:1 }; gameState.enemy = { left:1, right:1 }; gameState.playerTurn = true; gameState.pendingActiveUse = null; gameState.doubleMultiplier = 1; gameState.turnBuffs = []; gameState.equippedSkills = []; gameState.enemySkills = []; gameState.enemyDoubleMultiplier = 1; gameState.enemyTurnBuffs = []; gameState.baseStats = { playerThreshold:5, enemyThreshold:5, baseAttack:0, baseDefense:0, maxFinger:5 }; gameState.inBossReward = false; gameState.bossAbility = null; gameState.bossTurnCount = 0; gameState.enemyHasThirdHand = false; gameState.bossEnemyThresholdMultiplier = 1; gameState.playerBattleModifiers = { thresholdMultiplier:1, attackMultiplier:1, defenseMultiplier:1 }; gameState.playerShield = 0; gameState.awaitingEquip = false; selectedHand = null; equipTemp = []; removeOverlay(); if(unlockedList) unlockedList.style.display = 'none'; if(equippedList) equippedList.innerHTML = ''; messageArea.textContent = 'スキルをリセットしました（初期スキルに戻しました）'; showTitle(); }
...
function startBattle(){
  ...
  gameState.enemyTurnBuffs = [];
  if(gameState.isBoss){
    // choose a boss ability randomly for demo (or the game can set gameState.bossAbility earlier)
    const ability = BOSS_ABILITIES[rand(0, BOSS_ABILITIES.length - 1)];
    if(ability && ability.id){
      gameState.bossAbility = ability.id;
      engine.assignBossAbility(ability.id);
    }
  }
  assignEnemySkills();
...
/* ---------- BOSS ABILITIES list for assignment convenience ---------- */
const BOSS_ABILITIES = [
  { id:'reincarnation', name:'♻ 転生', desc:'破壊後に mod(maxFinger) で復活' },
  { id:'split', name:'✂ 分割', desc:'片手のみ生存で分裂' },
  { id:'thirdHand', name:'🖐 第三の手', desc:'戦闘開始時に第三の手が出現' },
  { id:'fortress', name:'🛡 超耐久', desc:'敵の閾値を×2（ボス戦中）' }
];
...
/* ---------- applyBattleStartPassives (possession etc.) ---------- */
...
function renderEquipped(){
  ...
  gameState.equippedSkills.forEach((s, idx) => {
    ...
    if(s.type === 'passive' || s.type === 'combo' || s.type === 'event' ){
      ...
      const btn = document.createElement('button'); btn.textContent = `${s.name} Lv${s.level}`; btn.disabled = s.used; if(s.used) btn.classList.add('used');
      btn.onclick = () => {
        if(gameState.inBossReward) return;
        if(s.used) return;
        playSE('skill', 0.7);
        if(s.id === 'double'){ s.used = true; gameState.doubleMultiplier = 1 + s.level; messageArea.textContent = `${s.name} を発動（次の攻撃が×${gameState.doubleMultiplier}）`; renderEquipped(); }
          ...
          else messageArea.textContent = 'このタイプのスキルは未実装';
        }
      };
...
function updateEnemySkillUI(){
  ...
  if(!gameState.enemySkills || gameState.enemySkills.length === 0){ enemySkillArea.textContent = '敵スキル: —'; return; }
  const typeColor = { passive: '#ddd', active: '#ffd166', turn: '#7cc7ff', combo: '#d7b3ff', event: '#ff9e9e' };
  const parts = gameState.enemySkills.map(s => { const cd = s.remainingCooldown && s.remainingCooldown > 0 ? ` (CD:${s.remainingCooldown})` : ''; const color = typeColor[s.type] || '#fff'; const def = SKILL_POOL.find(x=>x.id===s.id) || {}; const rar = def.rarity ? ` [${def.rarity.toUpperCase()}]` : ''; return `<span style="color:${color}; font-weight:700; margin-right:6px">${s.name}${rar} Lv${s.level}${cd}</span>`; });
  const buffs = gameState.enemyTurnBuffs.map(tb => { if(tb.skillId === 'fortify') return `防御+${tb.payload.value} (${tb.remainingTurns})`; if(tb.skillId === 'chain') return `次攻撃+${tb.payload.value} (${tb.remainingTurns})`; if(tb.skillId === 'teamPower') return `味方全体+${tb.payload.value} (${tb.remainingTurns})`; return ''; }).filter(Boolean);
...
function updateBossUI(){
  ...
  if(!gameState.isBoss || !gameState.bossAbility){ bossAbilityArea.textContent = ''; return; }
  const def = BOSS_ABILITIES.find(b=>b.id===gameState.bossAbility) || { name: gameState.bossAbility, desc:'' };
  bossAbilityArea.innerHTML = `<span style="color:#ff5555;font-weight:bold">BOSS能力: ${def.name}</span><br><small>${def.desc}</small>`;
}
function updateUI(){
...
function updateHand(key, value){
  ...
  else { displayThreshold = (gameState.baseStats && Number.isFinite(Number(gameState.baseStats.enemyThreshold))) ? Number(gameState.baseStats.enemyThreshold) : 5; displayThreshold *= (gameState.bossEnemyThresholdMultiplier || 1); }
...
function applyPendingActiveOnEnemy(side){
  ...
  if(pending.id === 'disrupt'){
    ...
    showPopupText(hands[key === 'left' ? 'enemyLeft' : (key === 'right' ? 'enemyRight' : 'enemyThird')], `-${amount}`, '#ff9e9e');
    sk.used = true;
...
/* ---------- player attack wrapper (calls engine) ---------- */
function playerAttack(targetSide){
  ...
  const res = engine.playerAttack(attackerKey, targetSide);
  ...
  if(res && res.destroyed && hasEquipped('chain')){ const lvl = getEquippedLevel('chain'); applyTurnBuff('chainBoost', lvl, 1); const tb = gameState.turnBuffs[gameState.turnBuffs.length - 1]; if(tb) tb.payload = { type:'chainBoost', value: lvl }; messageArea.textContent = `チェイン発動！次の攻撃が +${lvl}されます`; }
  ...
  if(gameState.isBoss && gameState.bossAbility && typeof engine.bossRegistry[gameState.bossAbility] !== 'undefined' && typeof engine.bossRegistry[gameState.bossAbility].onPlayerTurnEnd === 'function'){
...
/* ---------- enemy turn is handled by engine.enemyTurn() wrapper (used above) ---------- */
...
function checkWinLose(){
  ...
  if(playerDead){
    ...
    setTimeout(()=> { if(bestStageValue) bestStageValue.textContent = gameState.bestStage; showTitle(); }, 1000);
    return true;
  }
  return false;
}
...
function weightedRandomSkillFromList(list){ if(!list||list.length===0) return null; const total=list.reduce((s,x)=>s+getSkillWeight(x),0); let r=Math.random()*total; for(const s of list){ const w=getSkillWeight(s); if(r<w) return s; r-=w; } return list[0]; }

function generateBaseStatRewards(){ const pool=[ { id:'baseAttack', name:'⚔ 基礎攻撃 +1', desc:'全ての攻撃に恒久的に +1（ラン内有効）', apply:()=>{ gameState.baseStats.baseAttack=(gameState.baseStats.baseAttack||0)+1;} }, { id:'baseDefense', name:'🛡 基礎防御 +1', desc:'敵の攻撃に対する恒久的な防御 +1（ラン内有効）', apply:()=>{ gameState.baseStats.baseDefense=(gameState.baseStats.baseDefense||0)+1; } }, { id:'playerThreshold', name:'💎 破壊閾値 +1', desc:'指の破壊閾値を +1（プレイヤー側、ラン内有効）', apply:()=>{ gameState.baseStats.playerThreshold=(Number.isFinite(Number(gameState.baseStats.playerThreshold))?gameState.baseStats.playerThreshold:5)+1; } } ]; return pool.sort(()=>Math.random()-0.5).slice(0,3); }

function showBaseRewardSelection(rewards){ skillSelectArea.innerHTML = ''; messageArea.textContent = 'スキル報酬がありません。基礎能力を強化してください'; const wrap=document.createElement('div'); wrap.className='skill-choices'; rewards.forEach(r=>{ const btn=document.createElement('button'); btn.className='skill-btn node-btn'; btn.innerHTML=`<div style="font-weight:700">${r.name}</div><small style="opacity:.9">${r.desc}</small>`; btn.onclick=()=>{ playSE('click',0.6); r.apply(); messageArea.textContent=`${r.name} を獲得しました`; skillSelectArea.innerHTML=''; updateUI(); flashScreen(.14); setTimeout(()=>{ gameState.stage++; startBattle(); },700); }; wrap.appendChild(btn); }); skillSelectArea.appendChild(wrap); }

function showRewardSelection(){ const unlockedIds=(gameState.unlockedSkills||[]).map(u=>u.id); const notUnlocked=SKILL_POOL.filter(s=>!unlockedIds.includes(s.id)); const picks=[]; const tempPool=notUnlocked.slice(); while(picks.length<3 && tempPool.length>0){ const pick=weightedRandomSkillFromList(tempPool); if(!pick) break; picks.push({ id: pick.id, isNew:true }); const idx=tempPool.findIndex(x=>x.id===pick.id); if(idx!==-1) tempPool.splice(idx,1); } const getCap=(skillId)=>SKILL_LEVEL_CAP[skillId]||MAX_SKILL_LEVEL; const upgradeCandidates=(gameState.unlockedSkills||[]).filter(u=>{ const cap=getCap(u.id); return (u.level||1)<cap; }).map(u=>({ id:u.id, level:u.level, isUpgrade:true })); while(picks.length<3 && upgradeCandidates.length>0){ const u=upgradeCandidates.shift(); picks.push({ id:u.id, isUpgrade:true }); } if(picks.length<3){ const remainingNotUnlocked=SKILL_POOL.filter(s=>!picks.some(p=>p.id===s.id)); for(const s of remainingNotUnlocked){ if(picks.length>=3) break; if(!unlockedIds.includes(s.id)) picks.push({ id:s.id, isNew:true }); } } if(picks.length===0){ const baseRewards=generateBaseStatRewards(); showBaseRewardSelection(baseRewards); return; } skillSelectArea.innerHTML=''; messageArea.textContent='報酬スキルを1つ選んでください（永久アンロック / アップグレード）'; const wrap=document.createElement('div'); wrap.className='skill-choices'; picks.forEach(p=>{ const def=SKILL_POOL.find(s=>s.id===p.id); if(!def) return; const unlockedObj=gameState.unlockedSkills.find(u=>u.id===p.id); const label=p.isUpgrade ? `${def.name} を上昇 (現在 Lv${unlockedObj ? unlockedObj.level : 0})` : `${def.name} をアンロック`; const btn=document.createElement('button'); btn.className='skill-btn node-btn'; btn.classList.add('rarity-' + (def.rarity || 'common')); btn.innerHTML=`<div style="font-weight:700">${label}</div><small style="opacity:.9">${def.baseDesc}</small><div style="font-size:11px;opacity:.85;margin-top:6px">${(def.rarity||'common').toUpperCase()}</div>`; btn.onclick=()=>{ playSE('click',0.5); if(p.isUpgrade && unlockedObj){ const cap=getCap(def.id); unlockedObj.level=Math.min(cap,(unlockedObj.level||1)+1); messageArea.textContent=`${def.name} を Lv${unlockedObj.level} に強化しました`; } else { const cap=getCap(def.id); if(unlockedObj){ unlockedObj.level=Math.min(cap,(unlockedObj.level||1)+1); messageArea.textContent=`${def.name} を Lv${unlockedObj.level} に強化しました`; } else { gameState.unlockedSkills.push({ id: def.id, level: 1 }); messageArea.textContent=`${def.name} をアンロックしました！`; } } saveUnlocked(); skillSelectArea.innerHTML=''; flashScreen(.14); setTimeout(()=>{ gameState.stage++; startBattle(); },700); }; wrap.appendChild(btn); }); skillSelectArea.appendChild(wrap); }

/* ---------- boss reward selection ---------- */
function showBossRewardSelection(){ gameState.inBossReward=true; gameState.playerTurn=false; skillSelectArea.innerHTML=''; messageArea.textContent='ボス報酬を1つ選んでください（ラン内で恒久）'; const wrap=document.createElement('div'); wrap.className='skill-choices'; const options=[ { key:'playerThreshold', label:`指の閾値 +1 （現在 ${gameState.baseStats.playerThreshold}）` }, { key:'baseAttack', label:`基礎攻撃力 +1 （現在 ${gameState.baseStats.baseAttack}）` }, { key:'baseDefense', label:`基礎防御力 +1 （現在 ${gameState.baseStats.baseDefense}）` } ]; options.forEach(opt=>{ const btn=document.createElement('button'); btn.className='node-btn current'; btn.textContent=opt.label; btn.onclick=()=>{ playSE('click',0.6); if(opt.key==='playerThreshold'){ const cur=Number(gameState.baseStats.playerThreshold); gameState.baseStats.playerThreshold=Number.isFinite(cur)?(cur+1):6; } else { gameState.baseStats[opt.key]=(gameState.baseStats[opt.key]||0)+1; } messageArea.textContent=`${opt.label} を獲得しました`; gameState.bossEnemyThresholdMultiplier=1; gameState.bossAbility=null; gameState.enemyHasThirdHand=false; updateUI(); gameState.inBossReward=false; skillSelectArea.innerHTML=''; flashScreen(.18); setTimeout(()=>{ gameState.stage++; startBattle(); },700); }; wrap.appendChild(btn); }); skillSelectArea.appendChild(wrap); }

/* ---------- click handlers / hover overlay ---------- */
...
function setupHoverHandlers(){
  const mapping = [ { el: hands.enemyLeft, owner: 'enemy', hand: 'left' }, { el: hands.enemyRight, owner: 'enemy', hand: 'right' }, { el: hands.enemyThird, owner: 'enemy', hand: 'third' }, { el: hands.playerLeft, owner: 'player', hand: 'left' }, { el: hands.playerRight, owner: 'player', hand: 'right' } ];
...
function refreshOverlayContent(owner, hand){
  ...
  const destroyThreshold = engine.getDestroyThreshold(attackerIsPlayer);
  ...
  let pierceInfo = '';
  if(attackerIsPlayer){ const pierceLv = getEquippedLevel('pierce') || 0; if(pierceLv>0) pierceInfo = `（プレイヤーのピアス: Lv${pierceLv} が適用）`; } else { const enemyPierce = (gameState.enemySkills||[]).filter(s=>s.id==='pierce').reduce((sum,s)=>sum+(s.level||0),0); if(enemyPierce>0) pierceInfo = `（敵のピアス: Lv${enemyPierce} が適用）`; }
  const buffs=[];
  if(isEnemy){ (gameState.enemyTurnBuffs||[]).forEach(tb=>{ if(tb.payload && tb.remainingTurns>0){ if(tb.payload.type==='enemyGuardBoost' || tb.payload.type==='guardBoost') buffs.push(`防御+${tb.payload.value} (${tb.remainingTurns}ターン)`); if(tb.payload.type==='chainBoost') buffs.push(`次攻撃+${tb.payload.value} (${tb.remainingTurns}ターン)`); if(tb.payload.type==='teamPower') buffs.push(`味方全体+${tb.payload.value} (${tb.remainingTurns}ターン)`); } }); } else { (gameState.turnBuffs||[]).forEach(tb=>{ if(tb.payload && tb.remainingTurns>0){ if(tb.payload.type==='guardBoost') buffs.push(`防御+${tb.payload.value} (${tb.remainingTurns}ターン)`); if(tb.payload.type==='chainBoost') buffs.push(`次攻撃+${tb.payload.value} (${tb.remainingTurns}ターン)`); if(tb.payload.type==='teamPower') buffs.push(`味方全体+${tb.payload.value} (${tb.remainingTurns}ターン)`); } }); }
...
/* ---------- expose debug ---------- */
window.__FD = { state: gameState, engine, SKILL_POOL, commitEquips: ()=>commitEquips(), renderEquipped, assignEnemySkills, showBossRewardSelection, assignBossAbility: (id)=>engine.assignBossAbility(id), debug_getDestroyThreshold: (attacker)=>engine.getDestroyThreshold(attacker) };
